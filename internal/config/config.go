...
type Aggregation struct {
	...
	Static       []SwaggerSource     `mapstructure:"static"`
	Kubernetes   KubernetesDiscovery `mapstructure:"kubernetes"`
}
...
func setDefaults(v *viper.Viper) {
	v.SetDefault("app.name", "swagger-service")
	...
	v.SetDefault("database.schema", "swagger")
	...
	v.SetDefault("aggregation.fetch_timeout", "5s")
	v.SetDefault("aggregation.cache_ttl", "1m")
	v.SetDefault("aggregation.max_bytes", 8<<20)
	v.SetDefault("aggregation.static", []map[string]any{})
	v.SetDefault("aggregation.kubernetes.enabled", false)
	v.SetDefault("aggregation.kubernetes.namespace", "")
	v.SetDefault("aggregation.kubernetes.label_selector", "")
	v.SetDefault("aggregation.kubernetes.resync_period", "5m")
	v.SetDefault("jwt.issuer", "swagger-service")
	...
	v.SetDefault("auth.audience", "swagger-service")
	...
	v.SetDefault("migration.table", "swagger_schema_migrations")
...
func (c Config) Validate() error {
	...
	seenSources := make(map[string]struct{}, len(c.Aggregation.Static))
	for _, source := range c.Aggregation.Static {
		if strings.TrimSpace(source.Name) == "" || strings.TrimSpace(source.URL) == "" {
			return errors.New("aggregation.static entries require name and url")
		}
		if _, exists := seenSources[source.Name]; exists {
			return fmt.Errorf("aggregation.static contains duplicate name %q", source.Name)
		}
